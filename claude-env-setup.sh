...
if [ $# -lt 2 ]; then
    print_error "请提供必需的参数"
    echo "使用方法: $0 <base-url> <api-key>"
    echo "  base-url: API基础URL (必需)"
    echo "  api-key:  Anthropic API密钥 (必需)"
    exit 1
fi
...
detect_os_and_shell() {
    ...
    print_info "配置参数 - API Key: ****${ANTHROPIC_API_KEY: -4}, Base URL: $ANTHROPIC_BASE_URL"
...
EOF
    ...
    print_info "  ANTHROPIC_API_KEY: ****${ANTHROPIC_API_KEY: -4}"
}
...
verify_config() {
    ...
    if [ -n "$ANTHROPIC_BASE_URL" ] && [ -n "$ANTHROPIC_API_KEY" ]; then
        ...
        print_error "ANTHROPIC_BASE_URL: ${ANTHROPIC_BASE_URL:-"未设置"}"
        print_error "ANTHROPIC_API_KEY: ${ANTHROPIC_API_KEY:-"未设置"}"
        return 1
    fi
